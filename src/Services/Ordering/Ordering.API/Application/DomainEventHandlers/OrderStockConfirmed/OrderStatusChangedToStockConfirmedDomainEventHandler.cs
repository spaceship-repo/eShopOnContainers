                ...
                : INotificationHandler<OrderStatusChangedToStockConfirmedDomainEvent>
{
    ...
    public async Task Handle(OrderStatusChangedToStockConfirmedDomainEvent orderStatusChangedToStockConfirmedDomainEvent, CancellationToken cancellationToken)
    {
        ...
        //var paymentMethod = buyer.PaymentMethods.FirstOrDefault(x => x.Id == order.GetPaymentMethodId);

        var orderStatusChangedToStockConfirmedIntegrationEvent = new OrderStatusChangedToStockConfirmedIntegrationEvent(order.Id, order.OrderStatus.Name, string.Empty, string.Empty, JsonConvert.SerializeObject(order.OrderItems));
