...
public class OrderAggregateTest
{
    ...
    public void Add_event_Order_explicitly_raises_new_event()
    {
        ...
        //Act 
        var fakeOrder = new Order("1", "fakeName", new Address(street, city, state, country, zipcode), cardTypeId, cardNumber, cardSecurityNumber, cardHolderName, cardExpiration, "", "");
        fakeOrder.AddDomainEvent(new OrderStartedDomainEvent(fakeOrder, "fakeName", "1", cardTypeId, cardNumber, cardSecurityNumber, cardHolderName, cardExpiration, "", "", 0));
        //Assert
        Assert.Equal(fakeOrder.DomainEvents.Count, expectedResult);
    }
    ...
    public void Remove_event_Order_explicitly()
    {
        ...
        var cardExpiration = DateTime.Now.AddYears(1);
        var fakeOrder = new Order("1", "fakeName", new Address(street, city, state, country, zipcode), cardTypeId, cardNumber, cardSecurityNumber, cardHolderName, cardExpiration, "", "");
        var @fakeEvent = new OrderStartedDomainEvent(fakeOrder, "1", "fakeName", cardTypeId, cardNumber, cardSecurityNumber, cardHolderName, cardExpiration, "", "", 0);
        var expectedResult = 1;
